...
echo "=> Backup started"
if mongodump --host ${MONGODB_HOST} --port ${MONGODB_PORT} --username "${MONGODB_USER}" --password "${MONGODB_PASS}" --db "${MONGODB_DB}" --archive="${BACKUP_NAME}" --gzip && aws s3 cp "${BACKUP_NAME}" "${S3BACKUP}" && aws s3 cp "${S3BACKUP}" "${S3LATEST}" && rm "${BACKUP_NAME}"; then
...
if aws s3 cp \${S3RESTORE} \${RESTORE_ME} && mongorestore --uri "mongodb://${MONGODB_HOST}:${MONGODB_PORT}/${MONGODB_DB}${USER_STR}${PASS_STR}" --drop --archive=\${RESTORE_ME} --gzip && rm \${RESTORE_ME}; then
